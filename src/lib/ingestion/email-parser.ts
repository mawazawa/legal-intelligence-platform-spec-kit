...
export class EmailParser {
  ...
  private classifyEventType(parsed: any): EmailEvent['eventType'] {
    ...
    // Cooperation
    if (subject.includes('cooperate') || subject.includes('coordinate') ||
  ...
  private calculateCooperationScore(parsed: any): number {
    ...
    const positiveTerms = ['cooperate','work together','coordinate','collaborate','mutual','agreed','consensus','compromise','reasonable'];
    const negativeTerms = ['refuse','deny','object','oppose','unreasonable','stubborn','difficult','non-cooperative','refuse to'];
    positiveTerms.forEach(term => { if (body.includes(term) || subject.includes(term)) score += 0.1; });
    negativeTerms.forEach(term => { if (body.includes(term) || subject.includes(term)) score -= 0.1; });
  ...
  private calculateUrgency(parsed: any): EmailEvent['urgency'] {
    ...
    if (urgentTerms.some(t => subject.includes(t) || body.includes(t))) return 'high';
    if (mediumTerms.some(t => subject.includes(t) || body.includes(t))) return 'medium';
    return 'low';
  }
  ...
  async extractContinuances(): Promise<ContinuanceEvent[]> {
    ...
    return events
      .filter((e) => e.eventType === 'continuance_request')
      .map((event) => ({
        id: event.id,
        date: event.date,
  ...
  private extractDurationDays(event: EmailEvent): number {
    const body = event.body.toLowerCase();
    const match = body.match(/(\d+)\s*(day|week|month)/);
    if (match) {
